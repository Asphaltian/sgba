...
public class GbaDmaController
{
	...
	public void WriteControl( int ch, ushort value )
	{
		...
		if ( (value & 0x0800) != 0 )
			EmuLog.Write( LogCategory.Dma, LogLevel.Stub, "DRQ not implemented" );
		...
		if ( (c.NextSource & (width - 1)) != 0 && EmuLog.FilterTest( LogCategory.Dma, LogLevel.GameError ) )
			EmuLog.Write( LogCategory.Dma, LogLevel.GameError, $"Misaligned DMA source address: 0x{c.NextSource:X8}" );
		if ( (c.NextDest & (width - 1)) != 0 && EmuLog.FilterTest( LogCategory.Dma, LogLevel.GameError ) )
			EmuLog.Write( LogCategory.Dma, LogLevel.GameError, $"Misaligned DMA destination address: 0x{c.NextDest:X8}" );

		if ( EmuLog.FilterTest( LogCategory.Dma, LogLevel.Info ) )
			EmuLog.Write( LogCategory.Dma, LogLevel.Info,
		...
		{
			EmuLog.Write( LogCategory.Dma, LogLevel.Warn, "Discarding invalid DMA0 scheduling" );
		}
	}
