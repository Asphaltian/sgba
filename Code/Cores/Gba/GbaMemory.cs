...
public class GbaMemory
{
	...
	public void FlushAgbPrint()
	{
		...
		EmuLog.Write( LogCategory.Debug, LogLevel.Info, text );
	}
		...
		switch ( region )
		{
			case 0x0:
				if ( address < GbaConstants.BiosSize )
				{
					...
					EmuLog.Write( LogCategory.Memory, LogLevel.GameError, "Bad BIOS Load8: 0x{0:X8}", address );
					return (byte)(BiosPrefetch >> (int)((address & 3) * 8));
				}
				EmuLog.Write( LogCategory.Memory, LogLevel.GameError, "Bad memory Load8: 0x{0:X8}", address );
			...
			case 0x6:
				{
					...
					{
						EmuLog.Write( LogCategory.Memory, LogLevel.GameError, "Bad VRAM Load8: 0x{0:X8}", address );
						return 0;
					}
			...
			case 0xC:
				{
					...
					EmuLog.Write( LogCategory.Memory, LogLevel.GameError, "Out of bounds ROM Load8: 0x{0:X8}", address );
		...
		switch ( region )
		{
			case 0x0:
				if ( address < GbaConstants.BiosSize )
				{
					...
					EmuLog.Write( LogCategory.Memory, LogLevel.GameError, "Bad BIOS Load16: 0x{0:X8}", address );
					return (ushort)(BiosPrefetch >> (int)((address & 2) * 8));
				}
				EmuLog.Write( LogCategory.Memory, LogLevel.GameError, "Bad memory Load16: 0x{0:X8}", address );
			...
			case 0x6:
				{
					...
					if ( (rawAddr & 0x1C000) == 0x18000 && (Gba.Video.DispCnt & 7) >= 3 )
					{
						EmuLog.Write( LogCategory.Memory, LogLevel.GameError, "Bad VRAM Load16: 0x{0:X8}", address );
						return 0;
					}
			...
			case 0xC:
				{
					...
					EmuLog.Write( LogCategory.Memory, LogLevel.GameError, "Out of bounds ROM Load16: 0x{0:X8}", address );
		...
		switch ( region )
		{
			case 0x0:
				if ( address < GbaConstants.BiosSize )
				{
					...
					EmuLog.Write( LogCategory.Memory, LogLevel.GameError, "Bad BIOS Load32: 0x{0:X8}", address );
					return BiosPrefetch;
				}
				EmuLog.Write( LogCategory.Memory, LogLevel.GameError, "Bad memory Load32: 0x{0:X8}", address );
			...
			case 0x6:
				{
					...
					if ( (rawAddr & 0x1C000) == 0x18000 && (Gba.Video.DispCnt & 7) >= 3 )
					{
						EmuLog.Write( LogCategory.Memory, LogLevel.GameError, "Bad VRAM Load32: 0x{0:X8}", address );
						return 0;
					}
			...
			case 0xC:
				{
					...
					EmuLog.Write( LogCategory.Memory, LogLevel.GameError, "Out of bounds ROM Load32: 0x{0:X8}", address );
	...
	public void Store8( uint address, byte value )
	{
		...
		switch ( region )
		{
			...
			case 0x6:
				{
					...
					{
						EmuLog.Write( LogCategory.Memory, LogLevel.GameError, "Cannot Store8 to OBJ: 0x{0:X8}", address );
						break;
					}
				...
				break;
			case 0x7:
				EmuLog.Write( LogCategory.Memory, LogLevel.GameError, "Cannot Store8 to OAM: 0x{0:X8}", address );
				break;
			case 0x8:
				EmuLog.Write( LogCategory.Memory, LogLevel.Stub, "Unimplemented memory Store8: 0x{0:X8}", address );
				break;
			case 0x9:
			case 0xA:
			case 0xB:
			case 0xC:
			case 0xD:
				EmuLog.Write( LogCategory.Memory, LogLevel.GameError, "Bad memory Store8: 0x{0:X8}", address );
				break;
			case 0xE:
			case 0xF:
			...
			case 0x6:
				{
					...
					if ( (rawAddr & 0x1C000) == 0x18000 && (Gba.Video.DispCnt & 7) >= 3 )
					{
						EmuLog.Write( LogCategory.Memory, LogLevel.GameError, "Bad VRAM Store16: 0x{0:X8}", address );
						break;
					}
			...
			case 0x9:
				{
					...
					if ( region == 0x8 && romOffset >= 0xC4 && romOffset <= 0xC8 && (romOffset & 1) == 0 )
					{
						if ( Gba.Hardware.HasRtc )
							...
							EmuLog.Write( LogCategory.Hardware, LogLevel.Warn, "Write to GPIO address {0:X8} on cartridge without GPIO", address );
						break;
					}
					...
					EmuLog.Write( LogCategory.Memory, LogLevel.GameError, "Bad cartridge Store16: 0x{0:X8}", address );
					break;
				}
			case 0xA:
			case 0xB:
			case 0xC:
				EmuLog.Write( LogCategory.Memory, LogLevel.GameError, "Bad memory Store16: 0x{0:X8}", address );
				break;
			case 0xD:
				if ( Gba.Savedata.Type == SavedataType.Eeprom )
	...
	public void Store32( uint address, uint value )
	{
		...
		switch ( region )
		{
			...
			case 0x6:
				{
					...
					if ( (rawAddr & 0x1C000) == 0x18000 && (Gba.Video.DispCnt & 7) >= 3 )
					{
						EmuLog.Write( LogCategory.Memory, LogLevel.GameError, "Bad VRAM Store32: 0x{0:X8}", address );
						break;
					}
			...
			case 0xC:
			case 0xD:
				EmuLog.Write( LogCategory.Memory, LogLevel.Stub, "Unimplemented memory Store32: 0x{0:X8}", address );
				break;
		}
	}
	...
	private void HandleDebugFlags( ushort flags )
	{
		...
		EmuLog.Write( LogCategory.Debug, (LogLevel)level, msg );
	}
