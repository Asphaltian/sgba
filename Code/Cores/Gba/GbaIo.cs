...
public class GbaIo
{
	...
	{
		EmuLog.Write( LogCategory.Io, LogLevel.GameError, "Read from write-only I/O register: {0:X3}", offset );
		return (ushort)Gba.Cpu.LoadBadValue();
	}

	private ushort ReadUnusedRegister( uint offset )
	{
		EmuLog.Write( LogCategory.Io, LogLevel.GameError, "Read from unused I/O register: {0:X3}", offset );
		return (ushort)Gba.Cpu.LoadBadValue();
	}
	...
	public void Write16( uint offset, ushort value )
	{
		...
		switch ( offset )
		{
			...
			case 0x300:
				...
				{
					EmuLog.Write( LogCategory.Io, LogLevel.GameError, "Write to BIOS-only I/O register: {0:X3}", offset );
					writeIo = false;
					break;
				}
				...
				break;
			default:
				EmuLog.Write( LogCategory.Io, LogLevel.GameError, "Write to unused I/O register: {0:X3}", offset );
				writeIo = false;
				break;
		}
