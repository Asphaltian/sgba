...
public class GbaSavedata : ISaveData
{
	...
	public void ForceType( byte[] rom )
	{
		...
		if ( romText.Contains( "FLASH1M_V" ) )
		{
			...
			_currentBank = 0;
			EmuLog.Write( LogCategory.Save, LogLevel.Info, "Detected Flash 128K savegame" );
		}
		else if ( romText.Contains( "FLASH_V" ) || romText.Contains( "FLASH512_V" ) )
		{
			...
			Array.Fill( Data, (byte)0xFF );
			_currentBank = 0;
			EmuLog.Write( LogCategory.Save, LogLevel.Info, "Detected Flash 64K savegame" );
		}
		else if ( romText.Contains( "SRAM_V" ) || romText.Contains( "SRAM_F_V" ) )
		{
			...
			Array.Fill( Data, (byte)0xFF );
			EmuLog.Write( LogCategory.Save, LogLevel.Info, "Detected SRAM savegame" );
		}
		else if ( romText.Contains( "EEPROM_V" ) )
		{
			...
			Array.Fill( Data, (byte)0xFF );
			EmuLog.Write( LogCategory.Save, LogLevel.Info, "Detected EEPROM savegame" );
		}
		else
		{
			Type = SavedataType.None;
	...
	private void FlashSwitchBank( int bank )
	{
		...
		{
			EmuLog.Write( LogCategory.Save, LogLevel.Info, "Upgrading flash chip from 512kb to 1Mb" );
			Type = SavedataType.Flash1M;
